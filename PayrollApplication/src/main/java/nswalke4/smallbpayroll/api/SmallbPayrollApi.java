				...
				switch (method) {
					...
					case "GetAccountInfo":
						if (requestObj.has("AccountSub")) {
							responseObj = SmallbPayrollMethods
									.gatherAccountInformation(requestObj.getString("AccountSub"));
					...
					case "GetEmployeeTimecards":
						if (requestObj.has("EmployeeId")) {
							responseObj = SmallbPayrollMethods
									.gatherEmployeeTimecards(requestObj.getString("EmployeeId"));
							if (responseObj.has("Failure")) {
								response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
							} else {
								response.setStatus(HttpServletResponse.SC_OK);
					...
					case "GetPayPeriodTimecards":
						if (requestObj.has("PayPeriodId")) {
							responseObj = SmallbPayrollMethods
									.gatherPayPeriodTimecards(requestObj.getString("PayPeriodId"));
							if (responseObj.has("Failure")) {
								response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
							} else {
								response.setStatus(HttpServletResponse.SC_OK);
